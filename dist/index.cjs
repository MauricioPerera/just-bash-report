...
function siteHeader(config) {
  const logoHtml = config.brand?.logo ? `<img src="${escHtml(config.brand.logo)}" alt="Logo" class="site-logo"><br>` : "";
...
function buildReportCommand() {
  ...
  return (0, import_just_bash.defineCommand)("report", async (args, ctx) => {
    ...
    async function reportQuick(_exec, pos, fl, _ctx) {
      ...
      const validKinds = /* @__PURE__ */ new Set(["kpi", "chart", "table", "text"]);
      for (let i = 0; i < spec.sections.length; i++) {
        const s = spec.sections[i];
        if (!s || typeof s !== "object" || !("kind" in s)) {
          return fail(2, `section[${i}]: must have 'kind' field`);
        }
        if (!validKinds.has(s.kind)) {
          return fail(2, `section[${i}]: unknown kind '${s.kind}' (valid: kpi, chart, table, text)`);
        }
      }
    ...
    async function reportSite(exec2, pos, fl) {
      ...
      const rawDocs = JSON.parse(r.stdout);
      const posts = rawDocs.map((d) => ({
        _id: d._id,
        Title: d[fTitle] ?? "",
        Body: d[fBody] ?? "",
        Author: d[fAuthor],
        Status: d[fStatus] ?? "",
        Category: d[fCategory],
        Tags: d[fTags],
        PublishedAt: d[fDate],
        slug: d[fSlug],
